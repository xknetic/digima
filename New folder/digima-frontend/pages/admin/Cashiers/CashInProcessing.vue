...
<template>
  <div class="bg-white min-w-full h-full p-6 rounded-md shadow-md">
    <div class="flex justify-between items-center">
      <h3 class="text-2xl font-semibold text-gray-800">Cash In Process List</h3>

      <div class="flex items-center space-x-4">
        <SecondaryButton @click="showModalPlaceSlot = true">
          <p class="text-xs font-medium">Export as CSV</p>
        </SecondaryButton>

        <SecondaryButton @click="showModalCreateSlot = true">
          <p class="text-xs font-medium">Export as PDF</p>
        </SecondaryButton>

        <SecondaryButton @click="showModalLimit = true">
          <p class="text-xs font-medium">Cash-In Methods</p>
        </SecondaryButton>

        <SecondaryButton @click="showModalAddMember = true">
          <p class="text-xs font-medium">Process All Cash-In</p>
        </SecondaryButton>
      </div>
    </div>
      ...
      >
        <thead class="bg-slate-100 text-gray-700">
          <tr>
            ...
            <th class="px-4 py-2 border-b border-slate-100">Method</th>
            <th class="px-4 py-2 border-b border-slate-100">Currency</th>
            <th class="px-4 py-2 border-b border-slate-100">Amount Required</th>
            <th class="px-4 py-2 border-b border-slate-100">Cash In Charge</th>
            <th class="px-4 py-2 border-b border-slate-100">Cash In Status</th>
            <th class="px-4 py-2 border-b border-slate-100">Member Will Receive</th>
